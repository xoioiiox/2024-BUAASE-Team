...
export function render(map: Map, snake: Snake, food: Food, foodTwo: Food) {
  // 重置map
  reset(map);
  // 渲染蛇头
...
function _renderFoodTwo(map: Map, food: Food) {
  const row = food.y;
  const col = food.x;
  map[row][col] = -2;
}