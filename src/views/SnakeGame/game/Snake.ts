/*
 * @Description:蛇类 :  [-2 / -1 食物]  [1 蛇身体]  [2 蛇头]
 */
...
checkEat(food: Food, foodTwo: Food) {
    ...
    } else if(this.head.x === foodTwo.x && this.head.y === foodTwo.y) {
      if(foodTwo.isRightAns === true) {
        // 分数增加
        // this.scorePanel.addScore();
        // 食物的位置要进行重置
        food.change(this);
    ...
    switch (this.direction) {
      ...
      default:
        if (headY - 1 === bodyY && headX === bodyX) {
          moveDown(this.head, this.bodies);
          this.direction = 'Down';
        } else if (headY + 1 === bodyY && headX === bodyX) {
          moveUp(this.head, this.bodies);
          this.direction = 'Up';
        } else if (headY === bodyY && headX - 1 === bodyX) {
          moveRight(this.head, this.bodies);
          this.direction = 'Right';
          return;
        } else if (headY === bodyY && headX + 1 === bodyX) {
